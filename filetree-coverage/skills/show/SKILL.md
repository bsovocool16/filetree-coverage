...
description: >
  ...
  far each question got with every file (matched a search, opened and how
  much, named in the answer) over shading for the folders it searched or
  listed, a cumulative view across all questions, and
...
The map records tool calls, not judgment: "opened" was opened, "matched" means a search returned it, and a shaded folder means a search ran over it (a text search does not read PDFs or Word files in it). Whether an untouched file mattered is the reviewer's call; the page lets them mark expected files and lists the ones never opened.
...
2. `python3 ${CLAUDE_PLUGIN_ROOT}/scripts/build_coverage.py <room id> -o <out>/coverage.json [--expect-file f] [--merge 2-3] [--session id] [--since t]`
   `<out>`: `-o`, else `~/.claude/plugins/config/filetree-coverage/rooms/<id>/views/<YYYYMMDD-HHMM>/`. Each user prompt is one question, kept apart from other sessions recording into the same room; prompts that touched no file or folder are dropped (count printed). `--merge 2-3` (or `5,6`; repeatable; numbers as the unmerged map shows them) folds follow-up turns into one question: offer it when an answer names files a neighbouring turn opened. `--expect-file` is `{"q1": [globs], "*": [globs]}` relative to the room, only from what the reviewer said; never invent one.
...
5. Close with two or three sentences, in this order: files the answer names that nothing opened (the answer discusses a document it did not read; check it); expected files matched but not opened, or sitting unopened in a searched folder; files opened only in part, with the share; then, past one question, how many files no question has opened.
